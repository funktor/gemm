...
void gemm_mma_sync_fp16_swizzled(
    ...
    __shared__ alignas(16) half Mds[16*16];
    __shared__ alignas(16) half Nds[16*16];
        ...
        for (int j = 0; j < 16; j++) {
            for (int k1 = 0; k1 < k; k1 += 16) {
                ...
                for (int j1 = idx; j1 < 16*16; j1 += blockDim.x * blockDim.y) {
                    ...
                    Mds[row*16 + s_col] = a[(a_row + row) * k + (a_col + col)];
                }
                ...
                for (int j1 = idx; j1 < 16*16; j1 += blockDim.x * blockDim.y) {
                    ...
                    Nds[row*16 + s_col] = b[(b_row + row) * n + (b_col + col)];
                }
                ...
                for (int q = 0; q < 8; q += 2) {
                    ...
                    a_tile[q] = Mds[row*16 + s_col];
                    a_tile[q+1] = Mds[row*16 + s_col + 1];
                }
                ...
                for (int q = 0; q < 4; q += 2) {
                    ...
                    b_tile_1[q] = Nds[row*16 + s_col_1];
                    b_tile_2[q] = Nds[row*16 + s_col_1 + 8];

                    b_tile_1[q+1] = Nds[(row + 1)*16 + s_col_2];
                    b_tile_2[q+1] = Nds[(row + 1)*16 + s_col_2 + 8];
                }
                ...
                for (int q = 0; q < 4; q++) {
                    int rw = (thread_id_in_warp >> 2) + 8 * (q / 2);
                    int cl = 2 * (thread_id_in_warp % 4) + (q % 2);
                    c[(a_row + rw) * n + (b_col + cl)] += regs_c_1[q];
                    c[(a_row + rw) * n + (b_col + cl + 8)] += regs_c_2[q];
                }
                __syncthreads();
            }
        }
