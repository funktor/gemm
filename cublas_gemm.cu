...
void gemm_mma_sync_fp16(
    ...
    __shared__ alignas(16) half Nds[256];
    ...
    for (int i = 0; i < k; i += 16) {
        ...
        int b_row = i;
        int b_col = warp_col_id * 16;

        for (int j = idx; j < 256; j += blockDim.x * blockDim.y) {
            Nds[j] = b[(b_row + j/16) * n + (b_col + j % 16)];
        }
        ...
        #pragma unroll
        for (int j = 0; j < 4; j++) {
            int rw = (thread_id_in_warp >> 2) + 8 * (j / 2);
            int cl = 2 * (thread_id_in_warp % 4) + (j % 2);
            c[(a_row + rw) * n + (b_col + cl + 0)] += regs_c_1[j];
            c[(a_row + rw) * n + (b_col + cl + 8)] += regs_c_2[j];
        }
    }
